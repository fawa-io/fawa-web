...
import { greetClient } from "../api.ts";
...
export function useGreetService() {
  ...
  const runSayHello = async (name: string) => {
    ...
    addLog("--- Unary RPC ---");
    addLog(`Sending: ${name}`);
    const response = await greetClient.sayHello({ name });
  ...
  const runGreetStream = async (name: string) => {
    ...
    try {
      for await (const response of greetClient.greetStream({ name })) {
  ...
  const runGreetClientStream = async () => {
    ...
    const response = await greetClient.greetClientStream(sendNames());
    addLog(`Server replied with summary: ${response}`);
    addLog("--- Finished ---");
  };
  ...
  const runGreetBidiStream = async () => {
    ...
    try {
      const stream = greetClient.greetBidiStream(sendNames());
      for await (const response of stream) {
        addLog(`Received: ${response.echo}`);
      }
      addLog(`Bidi stream closed.`);
    } catch (e) {
      addLog(`Error: ${e instanceof Error ? e.message : "Unknown error"}`);
    }
    addLog("--- Finished ---");
