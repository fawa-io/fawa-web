...
export function useFileService() {
    ...
    const uploadFile = async (file: File) => {
        ...
        addLog(`Preparing to upload: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`);
        ...
        try {
            const response = await fileApiClient.sendFile(sendRequests());
    ...
    const downloadFile = async (fileName: string) => {
        ...
        try {
            const stream = fileApiClient.receiveFile({ fileName });
            let fileSize = 0;
            let receivedBytes = 0;
